...
import numpy as np
import importlib
from brainspy.utils.acceleration import format_numpy
...
def get_platform(platform):
    ...
    '''
    if platform['modality'] == 'single_chip':
        return SingleChip(platform)
    elif platform['modality'] == 'single_chip_simulation_nn':
        return SingleChipSimulationNN(platform)
    elif platform['modality'] == 'single_chip_simulation_kmc':
        return SingleChipSimulationKMC(platform)
    else:
...
class SingleChipSimulationNN:
    ...
    def evaluate_population(self, inputs_wfm, genePool, target_wfm):
...
if __name__ == '__main__':
    ...
    out = nn.evaluate_population(np.array([[0.3, 0.5, 0], [0.3, 0.5, 0], [0.3, 0.5, 0]]),
